...
uint8 DMA_A_initVar = 0u;


/*******************************************************************************
* Function Name: DMA_A_Start
********************************************************************************
*
* Summary:
...
void DMA_A_Start(void * srcAddress, void * dstAddress)
{
    ...
    CyDmaEnable();
    DMA_A_SetSrcAddress(0, srcAddress);
    DMA_A_SetDstAddress(0, dstAddress);
    DMA_A_ValidateDescriptor(0);
    DMA_A_ChEnable();
}


/*******************************************************************************
* Function Name: DMA_A_Init
********************************************************************************
*
* Summary:
*  Initializes the DMA channel based on the parameters set in the component
*  customizer. It is not necessary to call DMA_A_Init() because the
*  DMA_A_Start() API calls this function, which is the preferred method
...
{
    CYDMA_CH_CTL_BASE.ctl[DMA_A_CHANNEL] = DMA_A_CHANNEL_CFG;
    CYDMA_DESCR_BASE.descriptor[DMA_A_CHANNEL][0].ctl = DMA_A_DESCR0_CFG;
    CYDMA_DESCR_BASE.descriptor[DMA_A_CHANNEL][1].ctl = DMA_A_DESCR1_CFG;

    /* If the Generate IRQ option is selected for descriptor 0 or 1, set the
    * channel bit field in the DMA interrupt mask register.
    */
    #if (0u != ((DMA_A_DESCR0_CFG | DMA_A_DESCR1_CFG) & CYDMA_GENERATE_IRQ))
        CYDMA_INTR_MASK_REG |= DMA_A_CHANNEL_MASK;
    #endif /* (0u != ((DMA_A_DESCR0_CFG | DMA_A_DESCR1_CFG) & CYDMA_GENERATE_IRQ)) */
}
